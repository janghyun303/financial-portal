...
/* ─────────────────────────────────────────
   ...
   - 재무상태표: 계정과목 / 당기 / 전기 컬럼
   - 손익계산서: 계정과목 / 당기 / 전기 컬럼
   - 부가세신고: 기수/매출세액/매입세액/납부세액 등
   - 법인세신고: 과세표준/산출세액/납부세액 등
───────────────────────────────────────── */
const parseExcel = (buffer, docType) => {
  ...
  const toNum = (v) => {
    ...
    const s = String(v).replace(/,/g,"").replace(/\s/g,"").replace(/원/g,"").replace(/\((.+)\)/,"−$1");
    const n = parseFloat(s);
    return isNaN(n)?0:n;
  };

  if (docType === "재무상태표" || docType === "손익계산서") {
    // 헤더 행 찾기: "계정과목" 또는 "과 목" 이 들어있는 행
    let headerIdx = -1;
    ...
    for (let i=0; i<Math.min(raw.length,20); i++) {
      ...
      if (acIdx >= 0) {
        ...
        // 당기/전기 컬럼 탐색
        colCurrent = row.findIndex(c=>/당기|금액/.test(c));
        colPrev = row.findIndex(c=>/전기/.test(c));
        if (colCurrent < 0) colCurrent = acIdx+1;
        if (colPrev < 0) colPrev = acIdx+2;
        break;
      }
    }
    if (headerIdx < 0) {
    ...
    const rows = [];
    const totalKeywords = /총계|합\s*계|순이익|당기순|자본총|부채총|자산총/;
    const subtotalKeywords = /총이익|영업이익|차감전|소\s*계/;
    const headerKeywords = /^(유동|비유동|판매비|영업외|자 *본|부 *채|자 *산|매출액|매출원가)/;
    ...
    for (let i=headerIdx+1; i<raw.length; i++) {
      ...
      // 전부 0이고 계정명이 짧지 않으면 skip (소계 빈 행)
      if (cur===0 && prev===0 && acRaw.length < 2) continue;

      // 들여쓰기 레벨 추정 (앞 공백 or 특수문자)
      const leadSpace = (String(row[colAccount]||"").match(/^(\s+)/)||["",""])[1].length;
      const level = leadSpace > 4 ? 2 : leadSpace > 0 ? 1 : 0;

      let type = "item";
      if (totalKeywords.test(acRaw)) type = "total";
      else if (subtotalKeywords.test(acRaw)) type = "subtotal";
      else if (level===0 && headerKeywords.test(acRaw)) type = "header";
...
function Register({ onGo }) {
  ...
  const submit=()=>{
    ...
    setL(true);
    db.register({name:f.name,email:f.email,password:f.pw,phone:f.phone,memo:f.memo,businesses:vb.map(b=>b.no.trim())})
          ...
          <aside style={{width:"210px",background:T.bgDeepAlt,borderRight:`1px solid ${T.borderNav}`,padding:"20px 14px",flexShrink:0}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:"5px",marginBottom:"20px"}}>
              {years.map(y=>(
                <button key={y} onClick={()=>setYear(y)} style={{
                  ...
                  color:year===y?T.bgDeep:"rgba(255,255,255,0.55)",
            ...
            {MENUS.map(m=>(
              <button key={m.id} onClick={()=>setMenu(m.id)} style={{
                ...
                color:menu===m.id?T.bgDeep:"rgba(255,255,255,0.55)",
                ...
                onMouseLeave={e=>{if(menu!==m.id){e.currentTarget.style.background="transparent";e.currentTarget.style.color="rgba(255,255,255,0.55)";}}}
              >
                <Icon name={m.icon} size={15} color={menu===m.id?T.bgDeep:"rgba(255,255,255,0.6)"}/>
                <span>{m.label}</span>
              </button>
            ))}
          </aside>
